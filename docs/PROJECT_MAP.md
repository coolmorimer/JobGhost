...
Автопилот пропускает все существующие Application, не перегенерирует их и не сбрасывает NEEDS_REVIEW. Настройки: prepare_only (по умолчанию true), daily_limit (1–50, с дополнительным глобальным ограничением), excluded_companies/excluded_words (через запятую), letter_instructions (только стиль, не новые факты). Генерация использует выбранный AI API/браузер, доставка HH — браузер. Проверка чисел и заполнителей не доказывает достоверность всех утверждений; требуется проверка пользователем. ValueError показывается с исходной понятной причиной вместо общей ошибки капчи.

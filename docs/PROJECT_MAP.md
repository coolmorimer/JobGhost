...
Новый раздел **«Подготовка и тренировка»**: локальный Qdrant/FastEmbed, материалы, контекст вакансии, память API-сессии, AI Coach и план из 30 вопросов. Полные контракты, приватность и проверки — [PREPARATION.md](PREPARATION.md). Точки входа: `api/preparation.py`, `services/knowledge.py`, `services/context.py`, `pages/Preparation.tsx`. Qdrant хранится в RAM, SQLite — источник материалов; не добавляйте копию личных текстов в постоянный векторный индекс.
...
1. Пользователь выбирает «По резюме» или «Свой промпт» в настройках. Промпт и режим хранятся локально; смена во время сессии заблокирована.
2. `InterviewCapture.startSession()` вызывает `POST /api/ai/session/start`.
3. В режиме резюме backend читает активное резюме и удаляет контакты. В custom-режиме принимает custom_prompt, не читает и не отправляет резюме; пустой промпт отклоняется.
...
Оба контракта `/api/ai/session/start` и `/api/ai/ask` поддерживают `context_mode: resume|custom`, `custom_prompt`, nullable `resume_id`. В custom-режиме UI явно передаёт resume_id=null. Браузерный чат сохраняет старые сообщения: для полного удаления прежнего контекста нужен новый чат.
...
5. Ответ хранится в памяти страницы и текущей backend-сессии. Если пользователь явно включил сохранение, последние 30 текстовых пар сохраняются в SQLite; последние четыре входят в следующий запрос. В `done.sources` возвращаются найденные источники, UI показывает их под ответом.
...
3. Manual send требует явного подтверждения. Autopilot не отправляет отклики: он только готовит один новый черновик за цикл.
...
Автопилот читает рекомендации HH для выбранного резюме или обычную поисковую выдачу. Он пропускает все существующие Application, не перегенерирует их и не сбрасывает NEEDS_REVIEW. Настройки: source, prepare_only (принудительно true), daily_limit для черновиков, required_words, excluded_companies/excluded_words и letter_instructions (только стиль, не новые факты). Генерация использует выбранный AI API/браузер; при невалидной длине, заполнителях или неподтверждённых числах допускается одна ограниченная попытка переписать ответ, после чего валидация выполняется заново. Application создаётся только после успешной валидации. Доставка в HH доступна только из ручного подтверждённого сценария. Проверка чисел и заполнителей не доказывает достоверность всех утверждений; требуется проверка пользователем.

...
export function AnswerHistory({entries, index, onSelect,conversation=false,pendingQuestion,pendingAnswer,recognizedQuestion,recognizedSource,recognizedQuestionDetected=true}: {
  ...
  if(conversation)return <section ref={log} className="chat-log" role="log" aria-label="Переписка с помощником" aria-live="polite">
    ...
    {pendingQuestion&&<div className="chat-turn"><div className="chat-message user-message"><small>Вы</small><p>{pendingQuestion}</p></div><div className="chat-message assistant-message streaming-message" role="status"><small>Помощник</small><p>{pendingAnswer||'Начинаю отвечать…'}<span className="typing-caret" aria-hidden="true"/></p></div></div>}
  ...
  return <section ref={answer} aria-label="История ответов">
    ...
    <div className={pendingQuestion?'streaming-message':''} style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}} aria-live="polite">{entry.answer}{pendingQuestion&&<span className="typing-caret" aria-hidden="true"/>}</div>
